...
自动化覆盖 workflow 配置结构与流程图、事件隔离、按钮安全、单账号认证与会话权限、session 预检查、服务/账号锁、调度/退避/预算和日志脱敏。

## 真实环境验收

以下项目仍需要在目标 VPS 和一个用户自己的测试账号上执行，并记录日期、版本、结果和脱敏日志：

- [ ] 使用测试账号完成 `auth ACCOUNT_ID`，确认命令不依赖完整 `config.toml`，并确认 `/data/<account-id>.session` 权限为 `0600`；
